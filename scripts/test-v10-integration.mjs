...
const tempPath = path.join(root, 'scripts/.tmp-v10-combined.mjs');
const moduleSource = [
...
try {
  ...
  const oneMinute = {
    ...
    riskLevel: 'MEDIUM',
    historicalPattern: { validation: { samples: 58, accuracy: 47 }, currentPatternAt: '2026-08-03 18:00:00' },
  ...
  assert.ok(decision.targetProbability >= 52 && decision.targetProbability <= 68);
  assert.ok(decision.signalScore >= 52 && decision.signalScore <= 78);
  assert.equal(decision.cooldownMinutes, 30);

  console.log('✅ v10 combined decision integration passed');
