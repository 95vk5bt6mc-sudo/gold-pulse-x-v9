...
if [[ -n "$(git status --porcelain)" ]]; then
  fail "Working tree มีไฟล์ค้างอยู่ กรุณา Commit/Push งานเดิมให้เรียบร้อยก่อน เพื่อไม่ให้ patch ทับงานที่ยังไม่บันทึก"
fi
git status --short
...
old_vars = '''  const biasDirection = String(intelligence.bias?.direction || "WAIT").toUpperCase();
  const fakeBreakout = intelligence.fakeBreakout || {};
  const divergence = intelligence.divergence?.strongest || null;'''
new_vars = '''  const biasDirection = String(intelligence.bias?.direction || "WAIT").toUpperCase();
  const fakeBreakout = intelligence.fakeBreakout || {};
  const divergence = intelligence.divergence?.strongest || null;
...
require(old_vars in s or new_vars in s, "ไม่พบ overlay variable block")
...
gate_code = r'''  // MAIN TREND FIRST: trend-following is the default; counter-trend requires exceptional reversal evidence.
  if (["BUY", "SELL"].includes(originalDirection)) {
...
if p.exists():
    ...
    s = s.replace('if (pkg.version !== "11.0.0") {', 'if (!["11.0.0", "11.1.0"].includes(pkg.version)) {')
    s = s.replace('package version must be 11.0.0', 'package version must be 11.0.0 or 11.1.0')
    p.write_text(s)
